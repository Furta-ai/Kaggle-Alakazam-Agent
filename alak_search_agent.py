...
+ inlined 2-ply minimax search layer (determinization via cg search API).
"""
...
def heuristic_scores(obs):
    ...
    # ANTI-DECK-OUT (narrow): smetti di attivare pesca (Hilda/Dawn/Poke Pad/evolve-per-
    # pescare) SOLO quando (i) la mano attuale basta gia' per il KO del bersaglio
    # (target_can_kill e non serve pescare) E (ii) il mazzo e' ormai basso (safe_draws<8).
    # A mazzo pieno resta inerte -> nessun costo di tempo; frena solo verso il deck-out.
    overdraw = bool(target_can_kill and not need_dudunsparce_draw and safe_draws < 8)
...
N_DET = int(os.environ.get("ALAK_NDET", "3"))          # determinizations (env-tunable)
K_OPP = int(os.environ.get("ALAK_KOPP", "3"))          # opponent branching at ply-2 MAIN
MAX_SUBSTEPS = 40          # per greedy-complete rollout
TIME_BUDGET_S = float(os.environ.get("ALAK_TB", "0.80"))
SEARCH_MAX_OPTS = 24
_MAX_CAND = int(os.environ.get("ALAK_MAXCAND", "8"))   # numero di candidati valutati
...
# --- VALUE APPRESA (opzionale, via env): P(win) da XGBoost sui replay ladder ---
_LEARNED_VALUE = os.environ.get("ALAK_LEARNED_VALUE", "0") == "1"
_VAL_W = float(os.environ.get("ALAK_VAL_W", "4000"))       # peso della value appresa nel blend
_val_booster = None
_val_vf = None
class _StObs:                                              # shim: value_features usa solo obs.current
    __slots__ = ("current",)
    def __init__(self, st): self.current = st

def _load_val():
    global _val_booster, _val_vf
    if _val_booster is not None: return
    import sys as _s
    _old = os.environ.get("OMP_NUM_THREADS")
    os.environ["OMP_NUM_THREADS"] = "1"; os.environ["KMP_DUPLICATE_LIB_OK"] = "TRUE"
    _s.path.insert(0, os.path.join(os.path.dirname(__file__), "submission_old"))
    import xgboost as _xgb, model_common as _mc
    b = _xgb.Booster(); b.nthread = 1
    b.load_model(os.environ.get("ALAK_VAL_MODEL",
        "/private/tmp/claude-501/-Users-riccardoforte-Desktop/7c81ee9f-248b-48bf-b464-b34627c9b87b/scratchpad/ladder_value_xgb_tuned.json"))
    _val_booster = b; _val_vf = _mc.value_features

def _learned_pwin(state, me_i):
    _load_val()
    try:
        f = _val_vf(_StObs(state), me_i)
        return float(_val_booster.inplace_predict(f)[0])
    except Exception:
        return 0.5
...
def _leaf_eval(state, me_i):
    ...
    if _LEARNED_VALUE:
        # value appresa dominante + segnale-premi (grounding late) + safety no-active
        p = _learned_pwin(state, me_i)
        return _VAL_W * (2.0 * p - 1.0) + 1000.0 * prize_diff - 4000.0 * no_active
...
# --- Mazzo Alakazam di QUESTO agente (diverso da deck.csv del progetto). Il
#     registro (official_agents/make_vis) sovrascrive my_deck con questo dopo l'import. ---
ALAK_SEARCH_DECK = [
    5, 5, 19, 19, 19, 19, 65, 65, 66, 66, 66, 66, 305, 741, 741, 741, 741, 742, 742, 742,
    742, 743, 743, 743, 743, 1079, 1079, 1079, 1079, 1081, 1081, 1081, 1086, 1086, 1086, 1086,
    1097, 1097, 343, 1129, 1152, 1152, 1152, 1152, 1182, 1182, 1182, 1184, 1197, 1197,
    1225, 1225, 1225, 1225, 1227, 1231, 1231, 1231, 1231, 1247]   # 343=Shaymin (Flower Curtain, anti-danno panchina) al posto di 1 Night Stretcher
assert len(ALAK_SEARCH_DECK) == 60
