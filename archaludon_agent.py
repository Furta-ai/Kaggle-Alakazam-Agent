...
# ── Prize / lethal / anti-mill helpers ──

def prizes_remaining(ps):
    """Premi ancora da prendere (le carte-premio a faccia in giù = null)."""
    return len(ps.prize) if getattr(ps, "prize", None) else 0


def active_has_attack_option(obs):
    """C'è un'opzione ATTACCO disponibile ORA (attivo non bloccato e con energia)?"""
    return any(o.type == OptionType.ATTACK for o in obs.select.option)


def ready_bench_attacker(obs):
    """Un attaccante in panchina PRONTO (Archaludon/Duraludon con >=3 Metallo)?"""
    for p in my_state(obs).bench:
        if p and p.id in (ARCHALUDON_EX, DURALUDON) and energy_count(p) >= 3:
            return True
    return False


def ready_ko(obs):
    """Un attacco disponibile ORA mette KO l'attivo avversario?"""
    opp_act = opp_active_pokemon(obs)
    if not opp_act:
        return False
    for o in obs.select.option:
        if o.type == OptionType.ATTACK:
            if effective_damage(best_attack_damage(obs, getattr(o, 'attackId', None)), opp_act) >= opp_act.hp:
                return True
    return False


def attack_score(obs, attack_id):
    """Punteggio ATTACCO: la priorità NON è più solo il danno.
    - KO che vince la partita (premi presi >= premi che ci restano) -> 95000.
    - KO qualunque -> 22000 (batte pesca/ricerca: NON macinare il mazzo quando puoi
      prendere un premio; ex=2, mega ex=3 sono gestiti da prize_value).
    - Nessun KO ma siamo a fine partita (mazzo basso o pochi premi) -> almeno 21000
      (meglio picchiare che pescare fino al deck-out quando siamo avanti).
    - Altrimenti danno grezzo (setup prima, attacco per ultimo).
    """
    dmg = best_attack_damage(obs, attack_id)
    me = my_state(obs)
    opp_act = opp_active_pokemon(obs)
    my_pr = prizes_remaining(me)
    if opp_act and effective_damage(dmg, opp_act) >= opp_act.hp:
        if prize_value(opp_act) >= my_pr:
            return 95000
        return 22000
    if me.deckCount <= 8 or my_pr <= 3:
        return max(dmg, 21000)
    return dmg
...
def apply_overrides(obs, opt, score, reason):
    ...
    if opt.type == OptionType.ATTACK:
        ...
        if aid == METAL_DEFENDER:
            return -6000, "Crustle: Metal Defender does 0"
        if aid == RAGING_HAMMER:
            rh_dmg = 80 + damage_on(active_pokemon(obs)) // 10 * 10
            return max(score, 400), "Crustle: Raging Hammer"
